...
async def merchmind_exception_handler(request: Request, exc: MerchMindError) -> JSONResponse:
    ...
    return JSONResponse(status_code=status, content=_envelope(False, error=str(exc)))


async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    return JSONResponse(status_code=exc.status_code, content=_envelope(False, error=exc.detail))
