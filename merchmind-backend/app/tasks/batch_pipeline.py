...
def _generate_design_for_trend(self, trend_id: str, batch_id: str, pipeline_settings: dict):
    ...
    try:
        ...
        # Step 6b: Generate Printify mockup for t-shirt preview
        tshirt_product = db.query(Product).filter(
            Product.design_id == design.id, Product.product_type == "tshirt"
        ).first()
        if tshirt_product and (design.processed_image_url or design.raw_image_url):
            try:
                image_url = design.processed_image_url or design.raw_image_url
                from app.services.publishing.printify_publisher import create_product as printify_create, generate_mockups as printify_mockups, delete_product as printify_delete
                ...
                db.commit()
                logger.info(f"design_task[{trend_id[:8]}] Printify mockup generated for tshirt")
